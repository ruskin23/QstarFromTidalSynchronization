...
if __name__ == '__main__':
    ...
    teff_array=numpy.linspace(teff_min,teff_max,N)
    logg_array=numpy.linspace(logg_min,logg_max,N)
    feh_array=numpy.linspace(feh_min,feh_max,N)

    output_file='/mnt/md0/ruskin/QstarFromTidalSynchronization/binary_star_evolution/mass_calculations/teff_logg_feh.txt'

    with open(output_file,'w',1) as fo:
        fo.write('Teff' + '\t' + 'logg' + '\t' + 'feh' + '\n')
        for t in teff_array:
            for g in logg_array:
                for f in feh_array:

                    print('caclulating for ',t,g,f)

                    calculate_mass_age = Derive_mass(interpolator,t,g,f)
                    solution = calculate_mass_age()

                    if len(solution)==1:

                        primary_mass=solution[0][0]
                        ...
                        primary_mass_1=solution[0][0]
                        secondary_mass_1=primary_mass_1*q
                        age_1=solution[0][1]

                        primary_mass_2=solution[1][0]
                        secondary_mass_2=primary_mass_2*q
                        age_2=solution[1][1]
                        ...
                        fo.write(repr(t)+'\t'+
                                 ...
                                 repr(age_1)+'\t'+
                                 repr(primary_mass_1)+'\t'+
                                 repr(secondary_mass_1)+'\t'+
                                 repr(age_2)+'\t'+
                                 repr(primary_mass_2)+'\t'+
                                 repr(secondary_mass_2)+'\n')
                    else:
                        fo.write(repr(t)+'\t'+
                                 repr(g)+'\t'+
                                 repr(f)+'\t'+
