...
class evolution:
    ...
    def __init__(self,interpolator,observational_parameters):
        ...
        self.primary_mass, self.secondary_mass=self.calculate_star_masses()
    ...
    def __call__(self,q,sol_file,option=None):
        ...
        find_ic = InitialConditionSolver(disk_dissipation_age=tdisk,
            ...
            initial_p, initial_e, current_p, current_e, current_spin, delta_p, delta_e = find_ic(target=target, primary=primary,secondary=secondary)


        if option == 1:
            with open(fname,'a') as f:
                f.write(repr(self.logQ) + '\t' + repr(spin) + '\t' + repr(initial_p) + '\t'
...
if __name__ == '__main__':
    ...
    with open(data_file,'r') as f:
        ...
        for lines in f:
            ...
            sol_file='spin_vs_logQ_'+KIC+'.txt'
            with open(sol_file,'w') as fsol:
                fsol.write('#DATA:' + '\n' + '#KIC' + '\t' + 'Teff'  + '\t' +  'FeH'
            ...
            logQ = numpy.arange(9.0,11.0,0.5)
            for q in logQ:
                evolve(q,sol_file,task=1)

            pickle_fname='logQ_range_'+KIC+'.pickle'
            with open(pickle_fname,'rb') as fq:
                q_min=pickle.load(fq)
                q_max=pickle.load(fq)

            logQ_sol = brentq(
               lambda q:evolve(q,None,task=2)-Pspin,
               q_max.
               q_min)
