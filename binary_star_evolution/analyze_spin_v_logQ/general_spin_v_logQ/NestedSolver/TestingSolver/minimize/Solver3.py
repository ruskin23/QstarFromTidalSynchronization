...
class InitialConditionSolver:
    ...
    def _try_initial_conditions(self, initial_condition):
        ...
        return numpy.sqrt(((initial_condition[0]-self.orbital_period)**2)
                          *
                          ((initial_condition[1]-self.eccentricity)**2)
                          )
    ...
    def __call__(self, target, primary, secondary):
        ...
        sol=scipy.optimize.minimize(self._try_initial_conditions,
                                    [porb_target,e_target],
                                    method='L-BFGS-B',
                                    bounds=(((porb_target-5.0,porb_target+5.0),(e_target,0.4)))
                                    )
