...
if __name__ == '__main__':
    ...
    with open('KIC_catalog_2.txt','r') as f:
        ...
        for lines in f:
            ...
            print('Calculating for ', KIC)

            if feh>-1.014 and feh<0.537:

                m = interpolator.change_variables(feh,logg=float(data[9]),teff= float(data[1]))

                if len(m)==1:
                    p_mass=m[0][0]
                    age=m[0][1]
                    s_mass=q*p_mass
                    if s_mass>0.4 and p_mass<1.2:
                        f_out.write(KIC + '\t' + repr(age) + '\t' + repr(p_mass) + '\t' + repr(s_mass) + '\n')
                    ...
                    if (s_mass_1>0.4 or s_mass_2>0.4) and (p_mass_1<1.2 or p_mass_2<1.2):
                        f_out.write(KIC + '\t' +repr(age_1) + '\t' + repr(p_mass_1) + '\t' +
                                repr(s_mass_1) + '\t' + repr(age_2) + '\t' +
                                repr(p_mass_2) + '\t' +
                                repr(s_mass_2) + '\n')
