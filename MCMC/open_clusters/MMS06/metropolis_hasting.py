...
class MetropolisHastings:
    ...
    def values_proposed(self):
        ...
        proposed = dict()
        for key,value in self.sampling_parameters.items():
            if value['dist']=='TruncNormal':
                pdf=scipy.stats.truncnorm(value['lower'],value['higher'],loc=self.current_parameters[key],sigma=value['step'])
                proposed[key]=pdf.rvs()
            else:
                pdf=scipy.stats.norm(loc=self.current_parameters[key],sigma=value['step'])
                proposed[key]=pdf.rvs()
