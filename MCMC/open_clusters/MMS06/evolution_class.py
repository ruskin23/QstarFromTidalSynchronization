...
class evolution:
    ...
    def mass_equation(self,m2):

        return (((m2*numpy.sin(self.inclination))**1.5)/numpy.sqrt(self.mass_function)) - m2 - self.primary_mass

    def calculate_secondary_mass(self):

        try:
            sol=scipy.optimize.root_scalar(lambda m2: function(m2),bracket=[0.4,1.2],method='brentq')
            self.secondary_mass=sol.root
        except ValueError:self.secondary_mass=scipy.nan
        except: raise
