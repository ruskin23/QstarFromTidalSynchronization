        ...
        with open(fname3,'r') as f3:#MAth
            for line3 in f3:
                if re.search(kic,line3):
                    ...
                    spin_frequency=2*numpy.pi/pspin
                    R=orbital_frequency/spin_frequency
                    print(f'KIC = {kic} I = {R}')
                    if R<2:in_mode=in_mode+1
                    else:out_mode=out_mode+1
...
for i in range(len(KIC)-1):
    if Porb[i]<60:
        fc.write(repr(i+1) + '\t' + repr(KIC[i]) + '\t' + repr(teff[i]) + '\t' + repr(teff_e[i]) + '\t' + repr(feh[i]) + '\t' + repr(feh_e[i]) + '\t' + repr(Porb[i]) + '\t' + Porb_e[i] + '\t' + repr(logg[i]) + '\t' + repr(logg_e[i]) + '\t' + repr(Pspin[i]) + '\t' + repr(Pspin_e[i]) + '\t' + repr(q[i]) + '\n')


print(f'{in_mode} {out_mode}')
