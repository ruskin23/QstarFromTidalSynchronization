import corner
import numpy as np
import matplotlib.pyplot as plt
import sys

data=[]

system=sys.argv[1]
with open('AcceptedParameters.txt','r') as f:
    next(f)
    for lines in f:
        x=lines.split()
        data.append(float(x[1]))
        data.append(float(x[2]))
        data.append(float(x[3]))
        data.append(float(x[4]))
        data.append(float(x[5]))
        data.append(float(x[6]))
        data.append(float(x[7]))
        data.append(float(x[8]))
        data.append(float(x[9]))
        data.append(float(x[16]))

data=np.array(data)
d=data.reshape([len(data)//10,10])


figure=corner.corner(d,
                     labels=[r"$Porb$",r"ecosw",r"$esinw$",r"$M_p$",r"$M_s$",r"$age$",r"$feh$",r"$Wdisk$",r"$logQ$",r"$Pspin$"],
                     color='k',
                     show_titles=True)



plt.figure(1)
plt.show()
#plt.savefig('CornerPlot'+system+'.png')

